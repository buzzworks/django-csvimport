...
def save_bulkinsert(props=None, instance=None):
    """ To avoid circular imports do saves here """
    try:
        if not instance:
            from bulkinsert.models import CSVImport
            csvimp = CSVImport()
        if props:
...
class Command(LabelCommand):
    ...
    def handle_label(self, label, **options):
        ...
        errors = self.run()
        if self.props:
            save_bulkinsert(self.props, self)
    ...
    def run(self, logid=0):
        ...
        for row in self.csvfile[1:]:
            ...
            model_instance.bulkinsert_id = bulkinsertid
